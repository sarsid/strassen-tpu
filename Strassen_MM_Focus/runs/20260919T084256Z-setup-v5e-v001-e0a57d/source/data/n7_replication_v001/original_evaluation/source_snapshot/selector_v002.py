...
def fit(confirmation, selections_path, campaign_path):
    ...
    source_manifest = load(directory / "source_manifest.json")
    if source_manifest.get("sha256", {}).get("config_snapshot/" + campaign_path.name) != CONFIG_SHA256:
        raise ValueError("N5 confirmation did not use the frozen campaign configuration")
