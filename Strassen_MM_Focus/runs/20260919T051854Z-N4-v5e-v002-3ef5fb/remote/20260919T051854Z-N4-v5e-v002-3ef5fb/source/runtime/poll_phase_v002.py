#!/usr/bin/env python3
"""Read compact progress, transporting exact file bytes as base64 chunks."""
import argparse
import base64
import json
import os
...
def tail_from(path, offset, limit):
    if not path.exists():
        return {'offset': offset, 'next_offset': offset, 'bytes': 0,
                'data_base64': '', 'missing': True}
    size = path.stat().st_size
    if offset > size:
        return {'offset': offset, 'next_offset': offset, 'bytes': size,
                'data_base64': '', 'error': 'file_shrank_or_wrong_offset'}
...
def main():
    ...
    summary = None
    if raw_summary and not raw_summary.get('unavailable'):
    ...
    result = {'kind': 'phase_progress', 'transport': 'base64_bytes_v002', 'run_id': args.run_id,
