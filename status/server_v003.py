...
Version 003 preserves the earlier agent/run/commit views. Final result files
...
def compact_result(record, metadata, path, completion):
    ...
    selected = [item for item in comparison_list
                if item.get("reference_arm") == "native_xla"
    ...
    selected.sort(key=lambda item: (item.get("reference_arm") != "native_xla",
                                    item.get("reference_arm", "")))
    group_id = record.get("group_id", "")
    shape_id = record.get("shape_id") or group_id.split("__", 1)[0] or "unknown"
    ...
    return {
        ...
        "screen": phase == "N2", "accuracy_only": phase == "N4",
...
class Handler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if url.path == "/api/status":
            ...
            data = (ROOT / "status/index_v003.html").read_bytes()
...
if __name__ == "__main__":
    print("Status v003: http://127.0.0.1:8765", flush=True)
