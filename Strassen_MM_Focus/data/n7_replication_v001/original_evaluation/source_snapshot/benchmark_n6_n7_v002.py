    ...
    for group in groups:
        ...
        for scope in group["scopes"]:
            ...
            for name, row in eligible.items():
                ...
                pair = base.paired_comparison(other_samples, chosen_samples, group["inputs"][0]["seed"])
                if pair is not None:
                    pair["valid_numerical_comparison"] = selection_rule.passing(selected) and selection_rule.passing(row)
                paired[name] = pair
            fastest_family = {"native_xla": "native", "cubic_selected": "cubic", "strassen_selected": "strassen"}.get(fastest)
            result.append({"shape_id": group["shape"]["id"], "shape_mkn": [group["shape"][axis] for axis in ("m", "k", "n")],
                           ...
                           "fastest_observed_family": fastest_family,
                           "family_agreement_with_fastest_observed": (by_decision[group["shape"]["id"]]["label"] == fastest_family) if fastest_family else None,
