"""Render sealed N1-N9 evidence without executing or changing any benchmark.
...
def build(ev):
    ...
    for scope in ("call","prepared_kernel"):
        ...
        book.heat("n2-sweep-"+scope,"N2","N2 | Every tile and algorithm",n2,["N2"],
                  ...
                  takeaway="Tile choice changes performance. Crossed cells are VMEM-limit failures, not zero-time results.")
                    ...
                    for i,s in enumerate(c["series"]):
                        for j,cat in enumerate(c["categories"]):
                            ...
                            if p["value"] is None:
                                ...
                                ax.text(j,i,f'{p["value"]:.2g}'+suffix,ha="center",va="center",fontsize=7.2,color="white" if lum<.48 else "#162A43")
                else:
