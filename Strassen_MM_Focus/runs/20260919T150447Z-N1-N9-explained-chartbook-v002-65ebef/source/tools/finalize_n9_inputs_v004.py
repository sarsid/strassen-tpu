...
Version 004 recognizes the frozen application_prep_v001 seal layout: the
execution seal covers all artifacts and the preparation child has its own seal;
that adapter never emitted a canonical artifacts/artifact_manifest.json.
Tools and diagnostic adapters still require their canonical seal as well.
...
class Evidence:
    ...
    def run(self, path, *, successful=True, expected_completion=None):
        ...
        required = {'completion.json', 'artifacts/environment.json', 'artifacts/summary.json'}
        ...
        require(all('artifacts/' + relative in members for relative in actual_files),
                'Execution seal omits artifact files')
        canonical_path = path / 'artifacts/artifact_manifest.json'
        canonical = None
        if canonical_path.is_file():
            require('artifacts/artifact_manifest.json' in members, 'Canonical seal is absent from execution seal')
            canonical, inner = verify_seal(path / 'artifacts', 'artifact_manifest.json')
            require(set(inner) == actual_files, 'Canonical seal does not cover all artifact files')
            seal_layout = 'execution_plus_canonical_plus_child'
        else:
            # The original preparation adapter has no canonical seal. Admit
            # only its known single-action layout, with sealed source and args.
            require(summary.get('action') in ('download', 'corpus', 'tokenize', 'reference'),
                    'This adapter requires a canonical artifact seal')
            arguments = load(prior.sealed_file(path, 'artifacts/arguments.json'))
            source = prior.sealed_file(path, 'source/src/strassen_mm/application_prep_v001.py')
            frozen_source = Path(__file__).resolve().parents[1] / 'src/strassen_mm/application_prep_v001.py'
            require(prior.digest(source) == prior.digest(frozen_source), 'Preparation adapter source differs from frozen layout')
            require(arguments.get('action') == summary['action'] and arguments.get('phase') == summary.get('phase'),
                    'Preparation adapter action/phase mismatch')
            records = summary.get('records', [])
            require(len(records) == 1 and records[0].get('action') == summary['action'],
                    'Expected one original-adapter preparation action')
            command = records[0].get('command', [])
            require(len(command) >= 3 and PurePosixPath(command[1]).name == 'prepare_model_application_v001.py'
                    and command[2] == summary['action'], 'Unexpected preparation command')
            require((path / 'artifacts/preparation-00/artifact_manifest.json').is_file()
                    and not (path / 'artifacts/diagnostic').exists(), 'Missing original-adapter child seal or unexpected diagnostic')
            inner = {relative: members['artifacts/' + relative] for relative in actual_files}
            seal_layout = 'execution_plus_preparation_child_no_canonical_emitted'
        ...
        seals = {'execution': outer, 'canonical': canonical, 'layout': seal_layout}
