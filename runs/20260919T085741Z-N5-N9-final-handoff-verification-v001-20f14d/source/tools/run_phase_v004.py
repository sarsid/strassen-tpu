...
Version 004 adds versioned-module and extra-argument support and preserves timed-out controller calls, conservatively records launch
...
def main():
    ...
    parser.add_argument('--campaign-relative', default='configs/campaign_v1.json')
    parser.add_argument('--benchmark-module', default='strassen_mm.benchmark_v001')
    ...
    if not re.fullmatch(r'[A-Za-z0-9][A-Za-z0-9_.-]{0,80}', args.phase):
        parser.error('Invalid phase name')
    if not re.fullmatch(r'strassen_mm\.[a-z][a-z0-9_]*_v[0-9]{3}', args.benchmark_module):
        parser.error('Use a versioned strassen_mm module')
    ...
    label = args.phase + ('-real-weights' if args.benchmark_module.endswith('n4_real_weights_v001') else '') + '-v5e-v004'
    run = archive.begin(label, scope='tpu_benchmark')
    ...
    try:
        ...
        response = exec_remote('launch_phase_v004.py', [
            '--archive', remote_source, '--archive-sha256', digest(run / 'source.tar'),
            '--run-id', run.name, '--phase', args.phase,
            '--campaign-relative', args.campaign_relative, '--benchmark-module', args.benchmark_module,
