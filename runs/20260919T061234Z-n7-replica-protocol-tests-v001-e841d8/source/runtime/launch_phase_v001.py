...
def extract_source(archive, destination):
    ...
    matches = list(destination.rglob('src/strassen_mm/benchmark_v001.py'))
    if len(matches) != 1:
        raise ValueError('Expected exactly one src/strassen_mm/benchmark_v001.py in archive')
...
def launch(args):
    ...
    command = [sys.executable, '-u', '-m', 'strassen_mm.benchmark_v001',
    ...
    launcher = run / 'launch_phase_v001.py'
