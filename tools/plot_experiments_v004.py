"""Version 004: add per-shape descriptive best-value segments and diamonds.
...
def shape_label(key, shape):
    name = (key.replace("held_", "").replace("__frozen_rule", "")
            .replace("_swiglu", "").replace("_residual", "")
    ...
    with PdfPages(out/"N1_N9_key_comparisons.pdf",metadata=metadata) as pdf:
        ...
        for index,c in enumerate(book.charts):
            ...
            for mode in modes:
                ...
                fig.subplots_adjust(left=.28 if heat else .085,right=.92 if heat else .965,top=.80,bottom=.22 if not heat else .27)
                ...
                if heat:
                    ...
                    marker_kind=("mean time" if "time (" in c.get("ylabel","").lower() else
                                 "median error" if c["scope"]=="accuracy" and c.get("log") else None)
                    if marker_kind and c["kind"]=="bar":
                        for ci,cat in enumerate(c["categories"]):
                            eligible=[]
                            for si,s in enumerate(c["series"]):
                                p=points.get((cat["id"],s["id"]))
                                if not p or p["status"]!="ok" or p["value"] is None: continue
                                value=p["value"]
                                if mode=="relative":
                                    base=points.get((cat["id"],c["baseline"]))
                                    if not base or not base["value"]:continue
                                    value/=base["value"]
                                eligible.append((value,si))
                            if eligible:
                                best=min(v for v,_ in eligible)
                                ax.hlines(best,ci-.41,ci+.41,colors="#172433",linestyles=(0,(4,3)),linewidth=1.2,zorder=8)
                                for v,si in eligible:
                                    if math.isclose(v,best,rel_tol=1e-12,abs_tol=1e-15):
                                        xp=ci+(si-(nseries-1)/2)*bw
                                        ax.plot(xp,best,marker="D",ms=5,markerfacecolor="white",markeredgecolor="#172433",zorder=9)
                        fig.text(.045,.10,"Black dashed segment + diamond: lowest eligible "+marker_kind+" within each shape; descriptive, not a significance test.",fontsize=8,color="#172433")
                    ...
                    ax.set_xlim(-.55,ncat-.45)
                    if not values:
                        ax.set_ylim(0,1)
                        ax.set_yticks([])
                        ax.set_ylabel("No valid latency measurement")
                        reason="failed the VMEM limit" if all(p["status"]=="oom" for p in c["points"]) else "have no valid timing"
                        ax.text(.5,.52,f'All {len(c["points"])} configurations {reason}.',transform=ax.transAxes,
                                ha="center",va="center",fontsize=13,color="#B42318",fontweight="bold")
                    ax.set_xticks(xs,[x["label"] for x in c["categories"]],fontsize=8.5,
                                  rotation=25 if ncat>=8 and c["kind"]!="line" else 0,ha="right" if ncat>=8 and c["kind"]!="line" else "center")
