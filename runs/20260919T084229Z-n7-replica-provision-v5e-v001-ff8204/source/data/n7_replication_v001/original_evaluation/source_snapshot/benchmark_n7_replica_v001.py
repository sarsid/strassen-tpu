"""Fresh-v5e fixed-policy replication after original-cohort N9 completion.
...
def main(argv=None):
    ...
    parser.add_argument("--expected-identity", type=Path, required=True, help="New qualified smoke's environment.json")
    ...
    try:
        ...
        new_smoke_dir = args.expected_identity.parent
        smoke_dir, smoke_environment, smoke_results, smoke_provenance = verify_completed_run(new_smoke_dir, "smoke", 24)
        if smoke_dir / "environment.json" != args.expected_identity:
            raise ValueError("--expected-identity must name the new smoke's canonical environment.json")
        ...
        base.exclusive_json(out / "replication_inputs.json", {"original_evaluation": original_provenance,
            "new_smoke": smoke_provenance, "setup_identity_sha256": base.digest_file(args.setup_identity),
