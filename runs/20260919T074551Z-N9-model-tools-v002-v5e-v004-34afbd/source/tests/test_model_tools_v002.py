"""Offline recovery guards; no subprocess, network, ML imports or installations."""
...
class RecoveryGuards(unittest.TestCase):
    ...
    def test_existing_partial_and_escaped_venvs_are_rejected(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with patch.object(installer, 'PROJECT', root):
                ...
                with self.assertRaises(FileExistsError): installer.validate_new_target(root / 'model-tools-v001')
                with self.assertRaises(ValueError): installer.validate_new_target(root.parent / 'escaped-model-tools')
                (root / 'model-tools-v002').mkdir()
                with self.assertRaises(FileExistsError): installer.validate_new_target(root / 'model-tools-v002')
    ...
    def test_changed_checkpoint_bytes_or_original_completion_are_rejected(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with patch.object(resume, 'ROOT', root):
                with self.assertRaises(ValueError): resume.validate_reuse(bundle, corpus, endpoint)
    ...
    def test_wrong_corpus_split_and_endpoint_are_rejected(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            endpoint, bundle, corpus, seal, write = self.make_inputs(root)
            with patch.object(resume, 'ROOT', root):
                with self.assertRaises(ValueError): resume.validate_reuse(bundle, corpus, 'different-endpoint')
