#!/usr/bin/env python3
"""Create a new model-tools venv without ensurepip; manage it with global pip.
...
PINS=['transformers==4.56.2','tokenizers==0.22.0','huggingface-hub==0.34.4',
      'safetensors==0.6.2','numpy==2.2.6','pyarrow==21.0.0','sentencepiece==0.2.1','accelerate==1.10.1']
...
def validate_new_target(path):
    ...
    if not target.resolve().is_relative_to(PROJECT) or target.name=='model-tools-v001':
        raise ValueError('New environment must be inside the authorized project and preserve model-tools-v001')
    return target
...
from pathlib import Path
target=Path(sys.argv[1]).resolve()
names=['torch','transformers','tokenizers','huggingface-hub','safetensors','numpy','pyarrow','sentencepiece','accelerate']
versions={name:md.version(name) for name in names}
modules={}
for name in ['torch','transformers','tokenizers','huggingface_hub','safetensors','numpy','pyarrow','sentencepiece','accelerate']:
...
def main(argv=None):
    ...
    try:
        ...
        result={'venv_dir':str(target),'python':str(target/'bin/python'),'pins':['torch==2.8.0',*PINS],
                ...
                'strategy':'pip --python into fresh --without-pip venv; ensurepip bypassed',
