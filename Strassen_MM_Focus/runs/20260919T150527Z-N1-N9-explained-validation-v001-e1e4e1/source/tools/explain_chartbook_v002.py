"""Version 002: clarify the N7 comparator and use the corrected PDF assembler."""
...
def observation(c):
    ...
    if ranks:
        subject='the independently tuned Strassen alternative versus Native' if exp=='N7' else 'Strassen versus Native'
        result+=f"Archived paired intervals for {subject}: {ranks['faster']} faster, {ranks['slower']} slower, {ranks['inconclusive']} inconclusive."
...
def main():
    ...
    subprocess.run([args.pdf_python,str(Path(__file__).with_name('assemble_explained_pdf_v002.py')),
