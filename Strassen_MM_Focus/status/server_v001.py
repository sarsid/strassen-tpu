...
def state():
    ...
    for path in sorted((ROOT / "runs").glob("*/execution.json")):
        ...
        metadata["completed_cases"] = sum(r.get("kind") == "case_result" for r in rr)
        metadata["failed_cases"] = sum(r.get("kind") == "case_failure" for r in rr)
...
class Handler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if url.path == "/api/status":
            ...
            data = (ROOT / "status/index.html").read_bytes()
